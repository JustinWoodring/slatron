...
- `api/`: REST API endpoints organized by domain (auth, content, schedules, nodes, djs, scripts, bumpers, settings)
...
- Key tables: users, nodes, schedules, schedule_blocks, content_items, dj_profiles, ai_providers, scripts, bumpers
...
- `NodeState`: Shared state with schedule cache, MPV clients (main + voice), script cache, settings, bumper queue
